...
>>>>>>> 2c65c367e0dcd038c0b442c199461dbea0c2db1e
  ...
  return (
    <div className="fixed top-4 right-4 w-80 bg-black/40 backdrop-blur-md border border-purple-500/30 rounded-lg p-6 text-white/90 shadow-2xl">
      <h2 className="text-2xl font-bold mb-4 text-purple-300" style={{ fontFamily: 'Playwrite MX Guides, serif' }}>
        Infera
      </h2>
...
>>>>>>> 2c65c367e0dcd038c0b442c199461dbea0c2db1e
      ...
      {state && (
        <div className="space-y-3 pt-4 border-t border-purple-500/30">
          <h3 className="text-sm font-semibold text-purple-300 mb-2">Learned Parameters</h3>
          
          <div className="space-y-2 text-xs">
            <div className="flex justify-between">
              <span className="text-white/70">Gravity (g):</span>
              <span className="text-white font-mono">
                {state.g.toFixed(2)} ± {(Math.sqrt(latestParticle.filter.getCovariance().data[4][4])).toFixed(2)}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-white/70">Mass (m):</span>
              <span className="text-white font-mono">
                {state.m.toFixed(2)} ± {(Math.sqrt(latestParticle.filter.getCovariance().data[5][5])).toFixed(2)}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-white/70">Friction (μ):</span>
              <span className="text-white font-mono">
                {state.μ.toFixed(3)} ± {(Math.sqrt(latestParticle.filter.getCovariance().data[6][6])).toFixed(3)}
              </span>
            </div>
          </div>
      ...
      <div className="mt-6 pt-4 border-t border-purple-500/30 text-xs text-white/50">
        ...
        <p className="mt-1">Watch as uncertainty fades and motion stabilizes.</p>
      </div>
    </div>
  );
}
